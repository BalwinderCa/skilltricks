...
class CustomersController extends Controller
{
    ...
    // store customer
    public function store(CustomerRequestForm $request, OrganizationService $organizationService)
    {
        DB::beginTransaction();
        try {
            $customer = new User;
            ...
            // domain, so ownership of a domain goes to whoever was created
            // first rather than to whoever finishes calibrating first.
            $organizationService->attachUser($customer, $organizationService->resolveForUser($customer));
            if ($request->package) {
